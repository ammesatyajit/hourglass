...
- [~] **B2. Date-range validation + relative resolution.** ✅ RELATIVE-RESOLUTION half DONE 2026-06-16: added calendar `TimeWindow` tokens (`last_month`/`this_month`/`last_week`/`this_week`/`yesterday`/`today`) resolved in code via `Calendar.dateInterval` (PlanJSON.toDateRange); prompt routes bare "last month/week" → calendar form, "last N days" → rolling; fixed the misleading photo example that taught `last:30d`. Turned the photo query NEAR-PASS→PASS (rolling→calendar). ⬜ STILL OPEN: future-range clamp + corrective observation on backwards/future dates (the validation half).
