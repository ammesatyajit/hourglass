...
public extension NLAgent {
    ...
    static var toolLoopSystemPrompt: String {
        ...
        - For OPEN-ENDED / NO-KEYWORD queries with NO specific person named ("what have I been up to", "what have I been talking about", "catch me up", "what's new") — NOTE: if a person IS named, use the INVESTIGATIVE/readMessages path above instead — there is NO keyword to search — do NOT keyword-search a time phrase. Instead pull WHO you've been most active with: topContacts AND topGroups over the window (optionally overviewStats for volume), then synthesize by NAMING the top people/groups with their counts ("most active with Beck, Annika, and the Hao group…"). A bare message total ("23,231 messages") is NOT an acceptable answer to these — the user wants who/what, not just how much.
