...
## Timezone fix (2026-06-16, post-merge): resolveDateArg date-only args → LOCAL, not UTC
User request: "Make time zone current not utc." Closes one of the two DELIBERATE items left after the loop (the in:-arg UTC-vs-local audit). 
ROOT CAUSE (verified empirically + deterministically): the agentic `in:` resolver `resolveDateArg` parsed bare "YYYY-MM-DD" via `parseISODate`, which anchors date-only at UTC midnight. The `on:`/`before:`/`after:` operators (MessageSearch.swift:806, Calendar.current) and the UI are LOCAL. On this PDT (-0700) machine, in:"2026-06-14" actually resolved to 2026-06-13 17:00 → 2026-06-14 16:59 LOCAL — dropped June 14 evening, swept in June 13 evening. Two backends silently disagreed on what "June 14" meant.
FIX (Sources/NL/NLAgentReAct.swift): added `localDayStart` / `localDayEnd` (DST-safe: +1 day −1s) / `resolveBound(asHigh:)`. resolveDateArg now resolves date-only single-dates AND both range bounds via Calendar.current (low→00:00 local, high→23:59:59 local). TimeWindow tokens were ALREADY local (toDateRange uses Calendar). `parseISODate` LEFT UNCHANGED — messagesAroundTime depends on its UTC/explicit-offset parsing for absolute timestamps; the fix is localized to resolveDateArg.
VERIFICATION: (1) Deterministic snippet proved in:"2026-06-14" → exactly 2026-06-14 00:00:00..23:59:59 local, bit-for-bit == on:-operator Calendar.current.startOfDay (shift = +25200s = the −0700 offset). (2) Compiles + signs clean (full Hourglass.app Debug build ✓). (3) Unit tests: updated testResolveDateArg_acceptsExplicitISORange to expect local bounds; added testResolveDateArg_singleDate_anchorsToLocalDay. NOTE: `scripts/test.sh` XCTest run hung at host-app launch ("test runner hung before establishing connection", fixed ~344s timeout) — this is an XCTest-host bundle-injection/environment hiccup that fires BEFORE any test code runs (independent of this change; the app's XCTestConfigurationFilePath guards are intact at AppDelegate.swift:227). Did NOT run the live agentic eval: the user's Release app (PID 4069) was running and a headless model load would be a 2nd concurrent model process → OOM (hard constraint). Empirical UI verification deferred to user: relaunch + NL-query "how many did i send on june 14" → count moves from the UTC-skewed value to the local-day value.
Commit: 4bafbd8 (on main, local — NOT pushed). REMAINING deliberate item: agentic multi-term-OR under-return (tools.search objects; countMatching unaffected).

## Panel UI fix (2026-06-22): "No matching message found" no longer contradicts an aggregate answer
User screenshot: query `who'd i text the most in "Hao did this chat start" individually` rendered BOTH a confident answer ("You texted Beck Peterson the most … 2305 total messages (1077 sent, 1228 received)") AND "No matching message found." simultaneously.
ROOT CAUSE: `askAnswerView` in Sources/Panel/SpotlightPanel.swift showed the `explanation` banner, then in the `hero == nil` else-branch always printed "No matching message found." Aggregate answers (topContacts / topGroups / counts) have NO single hero message — explanation IS the answer — so the empty state contradicted it.
FIX (commit 7dc4750): gated the empty-state branch on `(result.explanation?.isEmpty ?? true)` → only show "No matching message found." when the agent produced no answer at all. Also removes the redundant double-message in ScopedPersonQuery's "I couldn't find any messages with X" path. Atomic UI-only change; build ✓.
NOTE — SEPARATE, NOT FIXED (model-compliance ceiling, logged #39 hao-group): the answer was UNSCOPED. The model called global `topContacts` (5 contacts) and phrased the result as if scoped to the named chat ("…the most in 'Hao did this chat start' individually"). The "in <chat>" scope was NOT applied — Beck Peterson + 2305 is a global-ish figure, not scoped to a "Hao" group. The query string itself is also malformed ("Hao did this chat start" isn't a clean chat reference). Group-scoped "who did I text most in <group> individually" remains a known small-model ceiling; not forced here. UI contradiction = fixed; scope-honoring = open/model-bound.

## Filter-on-aggregations, step 1 (2026-06-22): topContacts gains a chat scope
Triggered by the "Hao did this chat start" screenshot: a group-scoped "who'd I text the most in <group> individually" returned the user's GLOBAL #1 contact, phrased as if scoped — confidently wrong. User's framing: "filters should apply to everything, not just search." Correct: aggregation tools (topContacts/topGroups/overviewStats) only ever took a date range; the with:/from:/chat:/type: operator vocabulary was never plumbed into them (they're bespoke DashboardLoader SQL, separate from MessageSearch's parser). topContacts is ALSO 1:1-only (DashboardLoader.swift hardcodes ch.style=45) — it throws group messages out, so "rank a group's members" was not a filter away.
SHIPPED (commit 67d85bf) — first filter on an aggregation tool:
  • DashboardLoader.loadTopContacts: + `restrictToContactKeys: Set<String>?` (post-bucket filter) + shared static `contactIdentity(forHandle:contacts:)` so the bucket-key logic is single-sourced (callers pre-computing a key set can't drift).
  • NLToolsLive.topContacts(inChatNamed:in:limit:): resolves the named chat's members (chat_handle_join where display_name LIKE %name%) → their bucket keys → restricts the ranking. Empty match → empty result (agent told to report the miss, NOT go global). Protocol-extension default ignores scope so mocks/light conformers are untouched.
  • ReAct: parses optional `chat` arg on topContacts; observation labels the scope + on no-match instructs "do NOT fall back to a global ranking". Prompt tool #6 documents `chat`; one few-shot added.
SEMANTICS (told to model + user): because topContacts is 1:1-only, the scoped result = "of the people in <group>, who you text most ONE-ON-ONE", NOT who's most active inside the group. The in-group per-sender count is a SEPARATE, not-yet-built tool — offered to user as a fast follow-up if that's what they meant.
VERIFICATION: testTopContactsRestrictedToMemberKeys PASSES (one-key→that contact, counts intact; empty/unknown→empty). Full Debug build green. NOTE: XCTest host hang is INTERMITTENT — this run launched fine and the timezone resolveDateArg tests (4) ALSO pass now, retroactively confirming commit 4bafbd8. Did NOT run live agentic eval (user's app PID 93698 running → OOM). End-to-end agentic reliability (4B emitting the `chat` arg) is the usual compliance-ceiling risk; capability + deterministic path are proven. Commits local, NOT pushed.
OPEN/POSSIBLE NEXT: (a) in-group per-sender ranking tool if user wants "who posts most in the group"; (b) generalize filters to topGroups/overviewStats/countMatching (the full "filters everywhere" unification); (c) multi-term-OR under-return still open.

## Semantic-layer (dims/metrics/filters) value eval — 2026-06-22 (adversarial workflow, 5 agents)
Question: "see if a SQL-like dims/metrics/filters/joins semantic layer is useful before building." Ran a 5-agent adversarial Workflow (2 independent classifiers over 30 real+stretch queries → proponent vs skeptic → arbiter), all claims verified against source. No model runs (OOM-safe).
VERDICT: **build-minimal** (useful but NARROW; do NOT build the full mini-cube yet).
NUMBERS (both passes agree): ~23–24 of 30 queries = NO CHANGE. 6 genuine "spec-much-better" unlocks (S1 per-month-per-person, S3 per-contact-by-type, S9 per-month-sent-series, S10 in-group-by-sender, S11 per-group-by-type; +C15 in pass A) — ALL dim×filter compositions structurally impossible today (loadTopContacts hardcodes style=45 + date-only; loadTopGroups style=43 + date-only; neither group-by/filter-by-type). BUT 0 of the 6 unlocks are from the validated C-corpus — all are SPECULATIVE stretch queries (no observed demand). 5 queries (weekday/ratio/average/streak) out of scope for BOTH.
THE ONE REAL CORRECTNESS WIN = S10, and it's THIS SESSION's Hao query: the chat-scope I shipped (topContacts inChatNamed, commit 67d85bf) ranks members by their GLOBAL 1:1 volume with the user (post-aggregation filter at DashboardLoader L459-461 over the style=45 SQL), NOT their in-group activity — independently confirms the caveat I gave the user. "who posts most in <group>" is still WRONG today.
HONEST COSTS (skeptic, verified): the compiler RELOCATES chat.db quirks (handle→Contact in-memory merge, dual-epoch dates, stale-handle_id chj fix [recovered 89% of lost sent msgs], style 45/43, tapback exclusion) under one entry point — does not eliminate them. Free-text count (C8) must special-case BACK to materialized search or it REGRESSES (aggregateCount returns nil for non-empty phrase by design). RISKS: (1) 4B may not reliably emit a nested metric+dim+ParsedQuery spec — it already fails composing simpler flat operator strings (plans L4825); emission is flat scalar-only {tool,args}. (2) OPPORTUNITY COST: the open multi-term-OR under-return (tools.search=8 vs 1545 FTS/4528 INSTR, #42-#48, root cause UNKNOWN) degrades the content/synonym searches the REAL corpus (C3, C13) depends on — arguably fix FIRST.
RECOMMENDATION: STEP 1 now (high-conf): ship a targeted in-group per-sender ranking (dim:sender within a named chat via cmj/chat/handle joins) — fixes S10/C15, small emission target. = plans option (a). STEP 2 defer: full QuerySpec cube, gated on real analytics demand OR better 4B structured-emission reliability; when built, keep free-text-count → materialized search + retain bespoke tools as graceful-degradation fallback. SEPARATELY/first: the multi-term-OR bug. Workflow: wf_657df2eb-eac. Nothing built this turn (eval only).
